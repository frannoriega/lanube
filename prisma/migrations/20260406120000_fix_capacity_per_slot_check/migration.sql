  ...
  WHILE occ_ms <= recurrence_cap_ms LOOP
    ...
    ELSE
      ...
      FROM (
        ...
        FROM generate_series(occ_ms, (occ_ms + duration_ms) - 1, 900000::bigint) AS gs
        WHERE gs < (occ_ms + duration_ms)
      ) slot_check;
    ...
    ELSE
      ...
      FROM (
        ...
        WHERE p.reservation_id = overlap.reservation_id
          ...
          AND p.occurrence_start_time < res.end_time
          AND p.occurrence_end_time > res.start_time
      ) slot_check;
