...
/** Uploads a news post's cover/inline image through the active storage provider. */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { url } = await getStorage().upload({
      ...
      filename: file.name,
      folder: ["news", postId || "misc"],
    });
    return NextResponse.json({ url });
  } catch (e) {
    return apiServerError("admin/news/upload POST", e, { postId });
  }
}
