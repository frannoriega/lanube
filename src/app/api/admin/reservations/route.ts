...
export async function GET(request: NextRequest) {
  try {
    ...
    if (
      !allServices &&
      (!service || !ResourceType[service.toUpperCase() as keyof typeof ResourceType])
    ) {
    ...
    if (startDate != null && endDate != null) {
      ...
      const days = Math.min(31, Math.max(1, parseInt(forwardWindowRaw, 10) || ADMIN_RESERVATION_FORWARD_DAYS))
